    ...
    foreach ($rules as $ruleName => $rule):
        if ($rule['rule'] && !isset($rule['provider'])):
            $validationMethods[] = sprintf(
                "->add('%s', '%s', ['rule' => '%s', 'message' => __('The provided value for the {0} field is invalid', __('". Inflector::humanize($field) ."'))])",
                $field,
                $ruleName,
                $rule['rule']
            );
        elseif ($rule['rule'] && isset($rule['provider'])):
            $validationMethods[] = sprintf(
                "->add('%s', '%s', ['rule' => '%s', 'provider' => '%s', 'message' => __('The provided value for the {0} field is invalid', __('". Inflector::humanize($field) ."'))])",
                $field,
                $ruleName,
                $rule['rule'],
                $rule['provider']
        ...
        if (isset($rule['allowEmpty'])):
            ...
            elseif ($rule['allowEmpty']):
                ...
                $validationMethods[] = sprintf(
                    "->requirePresence('%s', 'create', __('This field {0} is required', __('". Inflector::humanize($field) ."')))",
                    $field
                );
                $validationMethods[] = sprintf(
                    "->notEmpty('%s', __('This field {0} is required', __('". Inflector::humanize($field) ."')))",
                    $field
                );
            endif;
        endif;
